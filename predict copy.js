...
export const predictDisease = async (symptoms) => {
  ...
  const input = preprocessInput(symptoms); // Convert symptoms to numeric format
  const tensorInput = tf.tensor2d([input]); // Shape [1, num_features]

  const predictionTensor = model.predict(tensorInput); // Model output
  const prediction = await predictionTensor.data(); // Get actual values
  const disease = postProcessPrediction(prediction); // Map to disease label
